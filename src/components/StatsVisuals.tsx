...
export function LeadSourceWidget({ leads }: StatsVisualsProps) {
  ...
  const segmentData = [
    { source: LeadSource.LINKEDIN, count: sourceGrouping[LeadSource.LINKEDIN] || 0, color: "#7C3AED", label: "LinkedIn Inbound" },
    { source: LeadSource.GOOGLE, count: sourceGrouping[LeadSource.GOOGLE] || 0, color: "#A855F7", label: "Google Organic" },
    { source: LeadSource.WEBSITE, count: sourceGrouping[LeadSource.WEBSITE] || 0, color: "#C084FC", label: "Website Referral" },
    { source: LeadSource.REFERRAL, count: sourceGrouping[LeadSource.REFERRAL] || 0, color: "#34D399", label: "Partner Referrals" },
    { source: LeadSource.COLD_OUTREACH, count: sourceGrouping[LeadSource.COLD_OUTREACH] || 0, color: "#F59E0B", label: "Outbound Outreach" },
    { source: LeadSource.TIKTOK, count: sourceGrouping[LeadSource.TIKTOK] || 0, color: "#FF0050", label: "TikTok Campaigns" },
    { source: LeadSource.FACEBOOK, count: sourceGrouping[LeadSource.FACEBOOK] || 0, color: "#1877F2", label: "Facebook Leads" },
    { source: LeadSource.INSTAGRAM, count: sourceGrouping[LeadSource.INSTAGRAM] || 0, color: "#E1306C", label: "Instagram Direct" },
    { source: LeadSource.YOUTUBE, count: sourceGrouping[LeadSource.YOUTUBE] || 0, color: "#FF0000", label: "YouTube Content" },
    { source: LeadSource.TWITTER, count: sourceGrouping[LeadSource.TWITTER] || 0, color: "#1DA1F2", label: "Twitter/X Sourcing" },
  ];
...
export function RevenueTrendWidget({ leads, formatCurrency }: { leads: Lead[]; formatCurrency?: (value: number) => string }) {
      ...
      <div className="h-10 px-3 bg-[#0D0B14]/75 border border-white/5 rounded-xl flex items-center justify-between text-xs">
        {hoveredIndex !== null ? (
          <>
            ...
            <span className="font-semibold text-white">Metrics Portfolio: <strong className="text-[#C084FC]">{formatCurrency ? formatCurrency(trendHistory[hoveredIndex].revenue) : `$${trendHistory[hoveredIndex].revenue.toLocaleString()}`}</strong></span>
