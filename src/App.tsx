...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (state.user) {
      // Auto-reconnect if sandbox session exists
      setCurrentUser(state.user);
      setIsLanding(false);
    }
  }, []);

  // Sync back to local persistence on updates
  useEffect(() => {
    if (currentUser) {
      saveState({
        leads,
  ...
  const handleLogout = () => {
    setCurrentUser(null);
    setIsLanding(true);
    // Erase auto-login state
    localStorage.removeItem("mip_user_v1");
  };

  // --- CRM LEAD MUTATORS ---
  const handleAddLead = (newLead: Omit<Lead, "id" | "dateCreated" | "lastUpdated">) => {
    const fullLead: Lead = {
      ...newLead,
      id: `lead-${Date.now()}`,
    ...
    // Push audit update log activity
  ...
  const handleUpdateLeadStage = (leadId: string, targetStage: FunnelStage) => {
    ...
    // Check if Win milestone is met to reflect Campaign conversions
    if (targetStage === FunnelStage.WON) {
      // Find campaign by tracking lead sources matching campaign name or simple matching
      setCampaigns(campaigns.map(camp => {
        // Simple mapping rule for sandbox simulation
  ...
  const handleLogLeadActivity = (leadId: string, description: string, type: any) => {
  ...
  const handleCreateLeadFollowup = (leadId: string, notes: string, scheduledTime: string, type: any, priority: any) => {
    const parentLead = leads.find(l => l.id === leadId);
    if (!parentLead) return;
  ...
  const handleAddNewCampaign = (newCamp: Omit<Campaign, "id" | "leadsCount" | "conversions" | "clicks" | "impressions">) => {
  ...
  const runAiGroundedQueries = async (customPrompt?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Backend query protocol error.");
      }
    ...
    <div className="flex h-screen bg-[#0D0B14] text-[#F8FAFC] font-sans overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                <div className="bg-[#1F1830] border border-white/5 p-4 rounded-2xl space-y-1.5 relative overflow-hidden group">
                  ...
                  <p className="text-2xl font-bold font-display text-[#C084FC]">${grossARRResult.toLocaleString()}</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <RevenueTrendWidget leads={leads} />
              </div>
                  ...
                  <div className="bg-[#161122] border border-[#7C3AED]/40 rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl relative">
                    ...
                    <form onSubmit={(e) => {
                      ...
                      <div className="grid grid-cols-2 gap-3 text-xs">
                        ...
                        <div className="space-y-1">
                          <label className="text-[10px] text-gray-400 uppercase">Expected Deal Value ($)</label>
                  ...
                  return (
                    <div key={stage} className="bg-[#161122]/90 border border-white/5 rounded-2xl w-72 shrink-0 p-4 space-y-3">
                      <div className="flex justify-between items-center pb-2 border-b border-white/5">
                        ...
                        <span className="font-mono text-[10px] text-[#C084FC] font-semibold">${stageSumValue.toLocaleString()}</span>
                      </div>
                          ...
                          stageLeads.map((lead) => (
                            <div key={lead.id} className="bg-[#1F1830] border border-white/5 hover:border-[#7C3AED]/30 p-3 rounded-xl space-y-3 text-xs transition-all relative">
                              <div className="flex justify-between items-start">
                                ...
                                <span className="font-mono font-semibold text-[#C084FC] block">${lead.value.toLocaleString()}</span>
                              </div>
          ...
          {activeTab === "analytics" && (
            <AnalyticsEngine leads={leads} campaigns={campaigns} />
          )}
